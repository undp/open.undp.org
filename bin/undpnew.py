...
projectsHeader = ['project_id','project_title','project_descr','start','end','inst_id','inst_descr','inst_type_id','inst_type_descr','subnational','operating_unit','operating_unit_id']
...
def loopData(file_name, key):
	...
	for event, p in context:
		for tx in p.findall("transaction"):
			transaction = parse_tx(tx)
		
		# hierarchy used to determine if output or input1
		...
		if hierarchy == '2':
			...
			subnat = p.findall('location')
			for loc in subnat:
				locationTemp = {}
				coordinates = loc.find('./coordinates').attrib
				locationTemp['lat'] = float(coordinates.get('latitude'))
				locationTemp['lon'] = float(coordinates.get('longitude'))
				locationTemp['type'] = loc.find('./location-type').text
				locationTemp['precision'] = coordinates.get('precision')
				locationTemp['name'] = loc.find('./name').text
				locationTemp['descrip'] = loc.find('./description').text
				subnationalTemp.append(locationTemp)
...
# 'subnatio 	nal']
			...
			# institutionid =  p.find("./")	value = tx.find('value')
			# 	if value is not None:
			# 		out['value_date'] = value.get('value-date')
		
			#institutionid = inst_descr.get('ref')
			#x = [ b.tag for b in p.iterfind("./participating-org[@role='Implementing']") ]
			
			# inst = (p.iterfind("./participating-org[@role='Implementing']"))
			...
			try: 
				...
				pass
			# projectList.append(inst_type_descr)
			projectList.append(subnationalTemp)
...
# # print root.tag
# # def loadActivity(activity):
# #     activity['description'] = list(activity['description'])
# #     activity['participating-org'] = list(activity['participating-org'])
# #     activity['addr_chg_nd'] = list(changeset['addr_chg_nd'])
# #     activity['addr_chg_way'] = list(changeset['addr_chg_way'])
...
# #     activity['details'] = dict(t.find('.//changeset').attrib)
